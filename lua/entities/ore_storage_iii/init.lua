...
function ENT:Initialize()
	...
	if not ply:IsAdmin() then self:SetModel("models/Slyfo/nacshuttleright.mdl") end
...
function ENT:CalcVars(ply)
	if not (ply.oremanage > 1) then self:Remove() return end
	self.IsOreStorage = true
	RD.AddResource(self, "ore", (4600000 + (ply.oremod_iii * 20000)) * ply.devlimit, 0)
end
