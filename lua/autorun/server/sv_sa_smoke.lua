/*OMGSTUPIDLUAERRORTHISWONTWORK
OMGSTUPIDLUAERRORTHISWONTWORK
OMGSTUPIDLUAERRORTHISWONTWORK*/

SA_SmokeSpeed = 200
SA_SmokeWindSpeed = SA_SmokeSpeed / 10

function SA_DeleteAllSmoke()
...
function SA_NewSmokeSpawner(id,startPos,endPosT,noWind,windAfter)
...
function SA_StopSmokeSpawner(id)
	if not SA_SmokeSpawners[id] then return end
	SA_SmokeSpawners[id] = nil
	timer.Destroy("SA_SpawnSmoke_"..id)
end

function SA_NewSmoke(id,startPos,endPosT,noWind,windAfter) --endPos is either VECTOR or NUMBER (height)
...
function SA_StopSmoke(id,doremove)
